...
@app.route(rule="/real_estate", methods=["POST"])
@token_required
def real_estate_agent(current_user):
    try:
        ...
        if not amount:
            return jsonify({"real_estate_payment_required":f"Hi {first_name}, payment is require!"}), 402

        if amount < price:
            return jsonify({"real_estate_payment_error":f"The minimum amount is:{price/1700:.2f}$."}), 403
        ...
        if response_data["status"]:
            return jsonify({"real_estate_response": response_data}), 200
        ...
        with db.engine.connect() as connection:
            ...
            connection.execute(statement=user_appointment, parameters={"first_name":first_name, "last_name":last_name, "gender":gender, "user_phone_number":user_phone_number, "address":address, "email_address":email_address, "next_of_kin":next_of_kin, "next_of_kin_phone_number":next_of_kin_phone_number, "next_of_kin_address":next_of_kin_address, "duration":duration, "price":price, "consultant":consultant, "location":location, "tel":tel, "organization_name":organization_name, "appointment_types":AppointmentTypes.REAL_ESTATE.value, "user_id":user["id"], "appointment_time":appointment_time, "appointment_date":appointment_date, "appointment_description":appointment_description})
            connection.commit()
            ...
            return jsonify({"real_estate_agent":"Real estate agent appointment was booked successfully!"}), 200
