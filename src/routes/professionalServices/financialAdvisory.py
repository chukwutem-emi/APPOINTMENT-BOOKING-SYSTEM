    ...
    try:
        ...
        for field in required_fields:
            if field not in data:
                return jsonify({"financial_adv_input_error":f"Missing required field:{field}"}), 400
        ...
        with db.engine.connect() as connection:
            ...
            if len(personnel_data) == 0:
                return jsonify({"message":"The financial-advisory personnel you selected doesn't exist or he/she might have been deleted from the database"}), 404
        ...
        return jsonify({"financial_advisory_kvError":f"Invalid input!:{str(KvError)}"}), 400
